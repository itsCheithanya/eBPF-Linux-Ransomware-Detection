...
with open('/home/pglabgpu/rans/eBPF-Linux-Ransomware-Detection/Finalyearproject/libbpf-bootstrap/examples/c/process_monitor_log.txt', "w") as f:
    f.truncate(0)
while True:
    # Read and accumulate logs
    accumulated_logs = read_accumulate_logs('/home/pglabgpu/rans/eBPF-Linux-Ransomware-Detection/Finalyearproject/libbpf-bootstrap/examples/c/process_monitor_log.txt')  # Replace 'logs.txt' with your log file path

    # print(accumulated_logs)
    ...
    for pid, logs in accumulated_logs.items():
        ...
        if df.shape[0]>MIN_ENTRIES:
          ...
          if most_frequent_element == "ransomware":
             ...
             print("PID {pid} terminated RANSOMWARE");        
             exit()
          # Check if any prediction is ransomware
          print(f"PID {pid} is {most_frequent_element}")
